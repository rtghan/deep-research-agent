...
def make_cfg(stability_rounds: int, sees_history: bool = False) -> Config:
    ...
    cfg.evolution.challenger_sees_history = sees_history
...
def run_condition(label: str, stability_rounds: int, sees_history: bool = False):
    cfg = make_cfg(stability_rounds, sees_history)
    state = build_state()
    ...
    print(f"\n{'='*74}\nCONDITION {label}  (stability_rounds={stability_rounds}, "
          f"challenger_memory={sees_history}, {n_claims} claims, evidence FROZEN)\n{'='*74}")
    ...
    for p in range(1, PASSES + 1):
        ...
        print(f"  pass {p}: challenged={challenged:3d}  changed={totals['changed']:3d}  "
              ...
              f"stalemate={stale:2d}  oscillating={osc}")

    return rows, n_claims


results = {}
for label, sr, hist in (("NO_MEMORY", 999, False), ("WITH_MEMORY", 999, True)):
    results[label] = run_condition(label, sr, hist)
...
no_mem = [r[2] for r in results["NO_MEMORY"][0]]
with_mem = [r[2] for r in results["WITH_MEMORY"][0]]
osc_no = max(r[6] for r in results["NO_MEMORY"][0])
osc_with = max(r[6] for r in results["WITH_MEMORY"][0])
print(f"\nrevisions per pass  no-memory {no_mem}  vs  with-memory {with_mem}")
print(f"total revisions     no-memory {sum(no_mem)}  vs  with-memory {sum(with_mem)}")
print(f"oscillating claims  no-memory {osc_no}  vs  with-memory {osc_with}")
print(f"final-pass revisions no-memory {no_mem[-1]} vs with-memory {with_mem[-1]}")
if sum(with_mem) < sum(no_mem) * 0.7:
    print("  => challenger memory MATERIALLY reduces churn: oscillation was a memory problem.")
elif sum(with_mem) < sum(no_mem):
    print("  => challenger memory reduces churn modestly; partially a memory problem.")
else:
    print("  => memory did NOT reduce churn: the evidence genuinely does not settle these claims.")

json.dump({k: {"rows": v[0], "n_claims": v[1]} for k, v in results.items()},
          open("eval/results/challenger_memory_convergence.json", "w"), indent=2)
print("\nSaved eval/results/challenger_memory_convergence.json")
